...
interface VendorCardProps {
  ...
  isOnline: boolean;
  scarcityCount?: number; // If <= 5, show scarcity indicator
  isWaiting?: boolean; // Vendor inactive state
...
export function VendorCard({
  ...
  return (
    <motion.div
      ...
      className={`rounded-[18px] border border-[#E2E8F0] overflow-hidden transition-all duration-150 ${
        isWaiting ? 'opacity-55 pointer-events-none' : 'bg-white shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05)]'
      }`}
    >
      ...
      <div className="p-4 pt-0 flex flex-col gap-3">
        ...
        <motion.button
          ...
          className={`h-14 w-full rounded-[10px] font-semibold text-white text-sm transition-all duration-150 ${
            isWaiting
              ? 'bg-[#9CA3AF] cursor-not-allowed'
              : 'bg-[#1E293B] hover:bg-[#0F172A] active:shadow-[inset_0_2px_4px_rgba(0,0,0,0.2)] cursor-pointer'
          }`}
        >
